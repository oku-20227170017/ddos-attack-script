#!/usr/bin/env python3
"""
Amaç: Laboratuvar ortamında 'low-and-slow' davranışları simüle etmek.
İçerir:
 - slow_read (HTTP)
 - idle_keepalive (HTTP)
 - dns_query_delay
...
GÜVENLİK ÖNLEMLERİ:
 - Default concurrency düşük (2)
 - Manuel durdurma dışında zaman sınırı yok
 - Hedef kullanıcıdan zorunlu olarak alınır
"""
...
async def slow_read(session: aiohttp.ClientSession, url: str, duration: int, read_chunk: int = 64, delay_between_chunks: float = 0.5):
    """
    GET isteği yap, yanıt geldiğinde küçük parçalarla oku ve her parça arasında gecik.
    Süresiz çalışır, Ctrl+C ile durdurulur.
    """
    while True:  # sürekli tekrar et
        try:
            ...
            await asyncio.sleep(1)  # hata olursa kısa bekle ve tekrar dene
...
async def main_async(args):
    ...
    await asyncio.gather(*tasks)  # timeout kaldırıldı

# ---------- CLI ----------
def parse_args():
    p = argparse.ArgumentParser(description="Lab-safe slow test simulator (no auto-timeout, target required)")
    p.add_argument("--scenario", choices=["slow_read", "idle_keepalive", "dns_query_delay", "tls_handshake_delay"], required=True)
    p.add_argument("--target", required=True, help="Target host or domain (required)")
    p.add_argument("--port", type=int, default=80)
    p.add_argument("--url", required=False, help="Full URL for HTTP scenarios (slow_read)")
...
def validate_args(args):
    ...
    if args.duration < 1:
        args.duration = 1
    if args.scenario == "tls_handshake_delay" and args.port == 80:
        print("[WARN] tls_handshake_delay usually uses port 443.")
    if not args.url and args.scenario == "slow_read":
        args.url = f"http://{args.target}/"
